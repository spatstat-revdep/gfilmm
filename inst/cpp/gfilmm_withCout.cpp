...
std::vector<double> fidSample(Eigen::VectorXd VT2, Eigen::VectorXd VTsum, 
  ...
  if((lhigh>0 && llow>0) || lzero>0){
    ...
    if(lzero==p){
      ...
      if((d1 && c1) || (d2 && c2)){
        ...
        for(size_t i=0; i<lhigh; i++){
          size_t hi = high[i];
          double xu = (U-VTsum.coeff(hi))/VT2.coeff(hi);
          double xl = (L-VTsum.coeff(hi))/VT2.coeff(hi);
        ...
        for(size_t i=0; i<llow; i++){
          size_t li = low[i];
          double xu = (U-VTsum.coeff(li))/VT2.coeff(li);
          double xl = (L-VTsum.coeff(li))/VT2.coeff(li);
    ...
    MIN = infty;
    for(size_t i=0; i<p; i++){
      double xu = (U-VTsum.coeff(i))/VT2.coeff(i);
      double xl = (L-VTsum.coeff(i))/VT2.coeff(i);
              ...
              for(size_t j=0; j<re; j++){
                ...
                for(size_t ii=0; ii<copy; ii++){
                  ...
                  if(rk < MAT.cols()){
                    ...
                    MM3 = O2aa + bbtau;
                    for(size_t jj=0; jj<lenZ1; jj++){
                      Ztemp[kk](Z00[jj],ii) = MM3.coeff(jj); 
                    }
