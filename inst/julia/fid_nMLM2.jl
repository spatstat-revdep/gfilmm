...
ESSS = cell(16)
srand(421)
fid_nMLM = function (data, FE, RE, N, thresh)
  ...
  if random_design == 1       # RE is declared.
    ...
    ESUM = cumsum(vec(E))'
    RE2 = [RE [1:n]]   #Adds the error effect
    RE = zeros(n, 0)
    for i = 1:ree #Builds an indicator matrix for the effects
      re_levels = unique(RE2[:, i])
      for j = 1:E[1, i]
        temp1 = find(RE2[:, i] .== re_levels[j])
        temp2 = zeros(n, 1)
        temp2[temp1, 1] = 1
        RE = [RE temp2]
      end
  ...
  Z = cell(ree, 1)        #Particles
  weight = cell(ree, 1)   #Weights
  ...
  for i = 1:ree  #Sample all the Z's and set-up the weights
    Z[i, 1] = randn(E[1, i], N)
  ...
  for i in 1 #:N erreur cisezski ou pas ??
    C[1, i] = zeros(Int, 0, 1)
    AA = [[FE; -FE] [A[i, 1]; -A[i, 1]]]
    AT = zeros(0, size(AA, 2))
    r = 0
    if rank(AA) != dim
  ...
  K = setminus(C[1, 1], 1:n)
  K_start = C[1, 1]
  Z[ree, 1][K, :] = 0  #remove error particles not selected in intitialization
  ...
  for k_n = 1:K_n
    ...
    for k in K_temp[1, k_n]
      ...
      if ESS[k, 1] < thresh && k < K[end] - 1
              ...
              for rr = 1:rep
                ...
                for kk in ord
                  for ii = 1:copy
                    ...
                    CO2 = RE[JJ, [ESUM[1, kk]-E[1, kk]+1:ESUM[1, kk]]]
        ...
        VC = VCVC
        CC = CCCC
        weight[ree, 1] = ones(E[1, ree], N)  #assign weights of error matrix to 1
      end
    ...
    for i = 1:N
      ...
      for kk in ord
        ...
        eff = setminus(kk, eff)
        for jj in eff
          XX = [XX RE[:, ESUM[1, jj]-E[1, jj]+1:ESUM[1, jj]-E[1, jj]+length(nn[jj, 1])] *
              Ztemp[jj, 1]]
        end
        #temp=find(RE2[:,kk].!=0)  #find which levels of kk have been sampled
        Z1 = Ztemp[kk, 1]  #Z being resampled
        CO2 = RE[:, ESUM[1, kk]-E[1, kk]+1:ESUM[1, kk]-E[1, kk]+length(nn[kk, 1])]  # c'?tait E[:,kk] mais une seule ligne
        level0 = find(sum(abs(CO2), 1) .== 0) #levels not sampled yet
        Z0 = find(Z1 .== 0)  #Z's not sampled
        Z00 = [1:length(Z1)]
