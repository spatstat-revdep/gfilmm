  ...
  if lcheckl != 0
    ...
    for ii in 1:lwhichl
      ...
      for dd in use
        ...
        lambda = (Lk - VTsum_wl[ii]) / (VTsum[checkl[dd]] - VTsum_wl[ii])
        VTtemp = hcat(
          VTtemp,
  ...
  if lchecku != 0
    ...
    for ii in 1:lwhichu
      ...
      for dd in use
        ...
        lambda = (Uk - VTsum_wu[ii]) / (VTsum[checku[dd]] - VTsum_wu[ii])
        VTtemp = hcat(
          VTtemp,
...
function gfilmm4(data::Array{R,2}, FE::Array{Float64,2}, RE::Array{Int64,2}, N::Int64, thresh::Float64) where {R<:Real}
  ...
  for i in 1:N
    ...
    AA = hcat(vcat(FE, -FE), vcat(A[i], -A[i]))
    b = vcat(U, -L) 
  ...
  for k_n in 1:K_n
    ...
    for k in K_temp[k_n]
      ...
      for i in 1:N
        ...
        VT2 = VT1[dim, :]  #value to be resampled
        ...
        VTsum = (Z1' * VT1)'
        (Z[ree][k, i], weight[k, i]) = fid_sample(VT2, VTsum, U[k], L[k]) ###Sample -
        # if wt == 0.0
        #   println("wt 0")
        # end
        VTsum += Z[ree][k, i] * VT2
      ...
      if ESS[k] < thresh && k < K[end]-1 #- 1 # -->> pourquoi -1 ?
        ...
        for i in 1:N
          if N_sons[i] > 0
            VCtemp = VC[i] * ones(Int64, N_sons[i])
            ...
            if copy > 0
                ...
                for kk in ord
                  for ii in 1:copy
                    ...
                    XX = XX[JJ, setdiff(1:size(XX, 2), kk)] #remove column of effect to be resampled
                    Z1 = Ztemp[kk][unique(RE2[JJ, kk]), ii]  #Z being resampled
    ...
    for i in 1:N
      ...
      for kk in ord
        ...
        CO2 = RE[:, (ESUM[kk]-E[kk]+1):(ESUM[kk]-E[kk]+length(nn[kk]))]
...
(VERTEX, WEIGHT, ESS) = gfilmm4(data, FE, RE, 5000, 2500.0)
